...
### T5 (레인 C) — 태그·어바웃·404 · 상태: DONE(2026-09-07) · 의존: T2
...
### T8 — 정책 마감 · 상태: DONE(2026-09-07) · 의존: T7
- 목표: draft 제외 전 경로 재확인, keystatic·React 런타임 dist 부재 검사 강화, 금지 문자열 스캔, 용량 예산 리포트.
- 완료 기준: [ ] verify 정책 항목 전부 그린 [ ] check 통과

### T9 — 마무리 통합 · 상태: DONE(2026-09-08) · 의존: T8
- 목표: 시드 콘텐츠 다듬기(발췌·태그 정리), 콘솔 경고 0, README 퀵스타트 실명령 갱신.
- 완료 기준: [ ] check + verify 전부 그린 [ ] `npm run preview` 기동 확인

### T10 — 사람 스모크 + 평가 메모 · 상태: DONE(2026-09-08, 시각 리뷰는 사람 확인 대기) · 의존: T9
- 목표: TESTING §5 스모크(Keystatic 편집→빌드 반영), `docs/EVAL-KEYSTATIC.md` 작성(편집 UX·스키마 표현력·패리티 유지 비용·실서비스 채택 판단), Vercel 배포 절차 문서화(실행은 선택).
